...
class GatewayRequestRedisBuffer:
    ...
    async def pop(self) -> GatewayRequestSnapshot:
        ...
        if popped is None:
            return _NO_COUNTS
        entries: Final = _BUFFERED_ENTRIES.validate_python(popped if isinstance(popped, list) else (popped,))
        return fold_counts(
            (
    ...
    async def commit_if_leader(self, prisma_client: "PrismaClient") -> None:
        """
        ...
        try:
            ...
            verbose_proxy_logger.warning(
                "Gateway request tracking - failed to commit %d buffered rows, re-queued to Redis for the next flush",
                len(buffered),
                exc_info=True,
            )
...
async def flush_gateway_requests(
    ...
    if redis_buffer is None:
        return
    try:
        await redis_buffer.commit_if_leader(prisma_client)
