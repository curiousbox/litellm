...
def member_may_write_auto_router(
    ...
    write (so neither a router nor a regular model can be converted); an existing row must be one
    the member created; and a public name the write registers must not widen the team's model access.
    """
    ...
    if effective_kind is None or (write.stored is not None and stored_kind is None):
        return False
