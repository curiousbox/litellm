...
def _make_router_mock_with_cooldown(
    ...
    whose ``get_model_ids`` returns ``routed_group_model_ids`` (the deployment ids
    that belong to the routed model group; defaulting to ``[]`` — origin absent
    from the routed group, i.e. a tier change).
    """
    ...
    mock_router.get_model_ids.return_value = list(routed_group_model_ids or [])
    return mock_router
...
async def test_affinity_honors_router_candidate_ids_for_team_and_pattern_routes():
    """
    The exact `model_name` index does not include team-public or pattern routes, so
    a same-group cooldown reached only through one of those would be misread as a tier
    change and stripped. The router stashes the deployment ids it actually resolved for
    the routed model on `request_kwargs`; the check must honor that set over the index.
    Here the routed group's index (`get_model_ids`) is empty, but the plumbed set marks
    the origin as a candidate, so the request fails fast with its reasoning intact.
    """
    from litellm.exceptions import ServiceUnavailableError
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    ...
    request_kwargs = {
        "litellm_metadata": {},
        "_routed_group_candidate_model_ids": frozenset({"deployment-team-a", "deployment-team-b"}),
        "input": [
            {"role": "user", "content": "why is the sky blue?"},
            {
                "type": "reasoning",
